...
static void
gjs_clear_locale_id (locale_t *id)
{
    if (id == NULL)
        return;
...
static locale_t
gjs_steal_locale_id (locale_t *id)
{
...
static gboolean
gjs_set_locale_id(locale_t *locale_id_pointer,
...
static int
gjs_locale_category_get_mask(GjsLocaleCategory category)
{
...
static size_t
get_number_of_locale_categories(void)
{
    return __builtin_popcount(LC_ALL_MASK) + 1;
}

static void
gjs_locales_free (GjsLocale **locales)
{
...
static GPrivate gjs_private_locale_key = G_PRIVATE_INIT((GDestroyNotify) gjs_locales_free);
...
gjs_setlocale(GjsLocaleCategory category, const char *locale_name)
{
    ...
    new_locale_id = newlocale(category_mask, locale_name, gjs_steal_locale_id (&old_locale_id));
    ...
    g_set_str(&locale->prior_name, g_steal_pointer(&prior_name));
