 ...
 * JSContext* and then trying to use it spontaneously from the main loop. I
 * don't think that's really quite kosher, and perhaps the problem is that (in
 * xulrunner) we just need to save a different context.
 *
 ...
 *
 * Here's the problem: this context can be destroyed. AFTER the context is
 * destroyed, or at least potentially after, the objects in the context's global
 * object may be garbage collected. Remember that JSObject* belong to a runtime,
 * not a context.
 *
 * There is apparently no robust way to track context destruction in
 * SpiderMonkey, because the context can be destroyed without running the
 * garbage collector, and xulrunner takes over the JS_SetContextCallback()
 * callback. So there's no callback for us.
 *
 * So, when we go to use our context, we iterate the contexts in the runtime and
 * see if ours is still in the valid list, and decide to invalidate the closure
 * if it isn't.
 *
 * The closure can thus be destroyed in several cases:
 * - invalidation by unref, e.g. when a signal is disconnected, closure is
 *   unref'd
 ...
 *
 * These don't have to happen in the same order; garbage collection can be
 * either before, or after, context destruction.
 */
...
/* Invalidation is like "dispose" - it is guaranteed to happen at finalize, but
 * may happen before finalize. Normally, g_closure_invalidate() is called when
 * the "target" of the closure becomes invalid, so that the source (the signal
 * connection, say can be removed.) The usage above in
 * global_context_finalized() is typical. Since the target of the closure is
 * under our control, it's unlikely that g_closure_invalidate() will ever be
 * called by anyone else, but in case it ever does, it's slightly better to
 * remove the "keep alive" here rather than in the finalize notifier.
 *
...
void Closure::closure_invalidated() {
    ...
    /* The context still exists, remove our destroy notifier. Otherwise we would
     * call the destroy notifier on an already-freed closure.
     *
     * This happens in the normal case, when the closure is invalidated for some
