...
bool gjs_flags_value_is_valid(JSContext* cx, GType gtype, int64_t value) {
    ...
    if (gtype != G_TYPE_NONE) {
        ...
        if (std::cmp_not_equal(
            ...
            gjs_throw(cx, "{:#x} is not a valid value for flags {}", value,
                      g_type_name(gtype));
            return false;
        }
    }
...
static bool gjs_enum_value_is_valid(JSContext* cx, const GI::EnumInfo& info,
    ...
    if (std::ranges::none_of(info.values(),
        ...
        gjs_throw(cx, "{} is not a valid value for enumeration {}", value,
                  info.name());
        return false;
    }
...
static bool gjs_array_to_g_list(JSContext* cx, JS::HandleValue value,
    ...
    for (size_t i = 0; i < length; ++i) {
        ...
        if (!JS_GetElement(cx, array, i, &elem)) {
            gjs_throw(cx, "Missing array element {}", i);
            return false;
        }
...
static bool hashtable_int_key(JSContext* cx, JS::HandleValue value,
    ...
    if (out_of_range) {
        gjs_throw(cx, "value is out of range for hash table key of type {}",
...
static bool value_to_ghashtable_key(JSContext* cx, JS::HandleValue value,
    ...
    gjs_debug_marshal(GJS_DEBUG_GFUNCTION,
                      "Converting JS::Value to GHashTable key {}", type_tag);

    if (!GI::is_supported_ghash_key_type(type_tag)) {
        gjs_throw(cx, "Type {} not supported for hash table keys", type_tag);
        return false;
    }
    ...
    switch (type_tag) {
        ...
        case GI_TYPE_TAG_INT8:
            return hashtable_int_key<int8_t>(cx, value, pointer_out);

        case GI_TYPE_TAG_INT16:
            return hashtable_int_key<int16_t>(cx, value, pointer_out);

        case GI_TYPE_TAG_INT32:
            return hashtable_int_key<int32_t>(cx, value, pointer_out);

        case GI_TYPE_TAG_UINT8:
            return hashtable_int_key<uint8_t>(cx, value, pointer_out);

        case GI_TYPE_TAG_UINT16:
            return hashtable_int_key<uint16_t>(cx, value, pointer_out);

        case GI_TYPE_TAG_UINT32:
            return hashtable_int_key<uint32_t>(cx, value, pointer_out);
...
bool gjs_array_to_strv(JSContext* cx, JS::Value array_value, unsigned length,
                       void** arr_p) {
    return gjs_array_to_auto_array<char*>(cx, array_value, length, arr_p);
}
...
static bool gjs_string_to_intarray(JSContext* cx, JS::HandleString str,
    ...
    switch (element_type) {
        ...
        case GI_TYPE_TAG_UINT8: {
            ...
            if (!gjs_string_to_utf8_n(cx, str, &result, length))
                return false;
        ...
        case GI_TYPE_TAG_UNICHAR: {
            ...
            if (!gjs_string_to_ucs4(cx, str, &result_ucs4, length))
                return false;
            *arr_p = result_ucs4;
            return true;
        }
        ...
        default:
            ...
            gjs_throw(cx, "Cannot convert string to array of '{}'",
                      element_type);
            return false;
    }
}
...
static bool array_to_basic_c_array(JSContext* cx, JS::HandleValue v_array,
    ...
    for (size_t ix = 0; ix < length; ix++) {
        ...
        if (!JS_GetElement(cx, array_obj, ix, &elem)) {
            gjs_throw(cx, "Missing array element {}", ix);
            return false;
        }
...
static bool gjs_array_to_ptrarray(JSContext* cx, JS::Value array_value,
    ...
    for (unsigned i = 0; i < length; i++) {
        ...
        elem = JS::UndefinedValue();
        if (!JS_GetElement(cx, array_obj, i, &elem)) {
            gjs_throw(cx, "Missing array element {}", i);
            return false;
        }
...
static bool gjs_array_to_basic_array(JSContext* cx, JS::HandleValue v_array,
    ...
    switch (element_storage_type) {
        ...
        case GI_TYPE_TAG_VOID:
            gjs_throw(cx, "Unhandled array element type {}",
...
GJS_JSAPI_RETURN_CONVENTION
static bool gjs_array_to_array(JSContext* cx, JS::HandleValue array_value,
    ...
    if (GI_TYPE_TAG_IS_BASIC(element_type)) {
        return gjs_array_to_basic_array(cx, array_value, length, element_type,
                                        arr_p);
    }
    ...
    switch (element_type) {
        ...
        default:
            ...
            gjs_throw(cx, "Unhandled array element type {}", element_type);
            return false;
    }
}
...
static void throw_invalid_argument(JSContext* cx, JS::HandleValue value,
                                   ...
                                   const char* arg_name,
                                   GjsArgumentType arg_type) {
    gjs_throw(cx, "Expected type {} for {} but got type '{:t}'", arginfo,
...
static bool throw_invalid_argument_tag(JSContext* cx, JS::HandleValue value,
                                       ...
                                       GjsArgumentType arg_type) {
    gjs_throw(cx, "Expected type {} for {} but got type '{:t}'", type_tag,
              gjs_argument_display_name(arg_name, arg_type), value);
    return false;
}
...
bool gjs_array_to_basic_explicit_array(
    ...
    gjs_debug_marshal(GJS_DEBUG_GFUNCTION,
                      "Converting argument '{}' JS value {} to C array",
                      arg_name, value);
...
bool gjs_array_to_explicit_array(JSContext* cx, JS::HandleValue value,
    ...
    if (GI_TYPE_TAG_IS_BASIC(element_tag)) {
        return gjs_array_to_basic_explicit_array(cx, value, element_tag,
    ...
    gjs_debug_marshal(
        GJS_DEBUG_GFUNCTION,
        "Converting argument '{}' JS value {} to C array, transfer {}",
        arg_name, value, transfer);

    if ((value.isNull() && !(flags & GjsArgumentFlags::MAY_BE_NULL)) ||
        (!value.isString() && !value.isObjectOrNull())) {
        throw_invalid_argument(cx, value, element_type, arg_name, arg_type);
        return false;
    }
    ...
    if (value.isString()) {
        // Allow strings as int8/uint8/int16/uint16 arrays
        JS::RootedString str{cx, value.toString()};
        return gjs_string_to_intarray(cx, str, element_tag, contents, length_p);
    }
    ...
    if (found_length) {
        ...
        if (!gjs_object_require_converted_property(cx, array_obj, nullptr,
                                                   atoms.length(), &length) ||
            !gjs_array_to_array(cx, value, length, transfer, element_type,
                                contents))
            return false;
...
static bool value_to_gdk_atom_gi_argument_internal(JSContext* cx,
    ...
    if (!value.isNull() && !value.isString()) {
        gjs_throw(cx, "Expected type String or null for {} but got type '{:t}'",
                  gjs_argument_display_name(arg_name, arg_type), value);
        return false;
    }
...
bool value_to_interface_gi_argument_internal(
    ...
    if (gtype != G_TYPE_NONE)
        gjs_debug_marshal(GJS_DEBUG_GFUNCTION, "gtype of INTERFACE is {}",
                          g_type_name(gtype));
    ...
    if (interface_info.is_enum_or_flags() == value.isObjectOrNull()) {
    ...
    if (value.isObject()) {
        ...
        if (gtype != G_TYPE_NONE) {
            ...
            gjs_throw(cx, "Unhandled GType {} unpacking GIArgument from Object",
                      g_type_name(gtype));
            gjs_arg_unset(arg);
            return false;
        }
        ...
        gjs_debug(GJS_DEBUG_GFUNCTION,
                  "conversion of JSObject value {} to type {} failed", value,
                  interface_info.name());
    ...
    if (value.isNumber()) {
        ...
        gjs_throw(cx, "Unhandled GType {} unpacking GIArgument from Number",
                  g_type_name(gtype));
        return false;
    }

    gjs_debug(GJS_DEBUG_GFUNCTION,
              "JSObject type '{:t}' is neither null nor an object", value);
...
inline static bool gjs_arg_set_from_js_value(JSContext* cx,
    ...
    if (!gjs_arg_set_from_js_value<TAG>(cx, value, arg, &out_of_range)) {
        if (out_of_range) {
            gjs_throw(cx, "value {} is out of range for {} (type {})", value,
    ...
    gjs_debug_marshal(GJS_DEBUG_GFUNCTION, "{} set to value {} (type {})",
                      gjs_argument_display_name(arg_name, arg_type),
...
static bool check_nullable_argument(JSContext* cx, const char* arg_name,
    ...
    if (!(flags & GjsArgumentFlags::MAY_BE_NULL) && !gjs_arg_get<void*>(arg)) {
        gjs_throw(cx, "{} (type {}) may not be null",
                  gjs_argument_display_name(arg_name, arg_type), type_tag);
        return false;
    }
...
bool gjs_value_to_basic_gi_argument(JSContext* cx, JS::HandleValue value,
    ...
    gjs_debug_marshal(
        GJS_DEBUG_GFUNCTION,
        "Converting argument '{}' JS value {} to GIArgument type {}", arg_name,
        value, type_tag);
...
bool gjs_value_to_gerror_gi_argument(JSContext* cx, JS::HandleValue value,
    ...
    gjs_debug_marshal(
        GJS_DEBUG_GFUNCTION,
        "Converting argument '{}' JS value {} to GIArgument type error",
        arg_name, value);
...
bool gjs_value_to_gdk_atom_gi_argument(JSContext* cx, JS::HandleValue value,
    ...
    gjs_debug_marshal(
        GJS_DEBUG_GFUNCTION,
        "Converting argument '{}' JS value {} to GIArgument type interface",
        arg_name, value);
...
bool gjs_value_to_basic_glist_gi_argument(JSContext* cx, JS::HandleValue value,
    ...
    gjs_debug_marshal(
        GJS_DEBUG_GFUNCTION,
        "Converting argument '{}' JS value {} to GIArgument type glist",
        arg_name, value);
...
bool gjs_value_to_basic_gslist_gi_argument(JSContext* cx, JS::HandleValue value,
    ...
    gjs_debug_marshal(
        GJS_DEBUG_GFUNCTION,
        "Converting argument '{}' JS value {} to GIArgument type gslist",
        arg_name, value);
...
bool gjs_value_to_basic_ghash_gi_argument(
    ...
    gjs_debug_marshal(
        GJS_DEBUG_GFUNCTION,
        "Converting argument '{}' JS value {} to GIArgument type ghash",
        arg_name, value);
...
bool gjs_value_to_gi_argument(JSContext* cx, JS::HandleValue value,
    ...
    if (type_info.is_basic()) {
        return gjs_value_to_basic_gi_argument(cx, value, type_tag, arg,
                                              arg_name, arg_type, flags);
    }

    if (type_tag == GI_TYPE_TAG_ERROR) {
        return gjs_value_to_gerror_gi_argument(cx, value, transfer, arg,
                                               arg_name, arg_type, flags);
    }

    if (type_tag == GI_TYPE_TAG_INTERFACE) {
        GI::AutoBaseInfo interface_info{type_info.interface()};
        return gjs_value_to_interface_gi_argument(cx, value, interface_info,
                                                  transfer, arg, arg_name,
                                                  arg_type, flags);
    }
    ...
    if (type_tag == GI_TYPE_TAG_GLIST || type_tag == GI_TYPE_TAG_GSLIST) {
        ...
        if (element_type.is_basic()) {
            if (type_tag == GI_TYPE_TAG_GLIST)
                return gjs_value_to_basic_glist_gi_argument(
                    cx, value, element_type.tag(), arg, arg_name, arg_type);
        ...
        if (key_type.is_basic() && value_type.is_basic()) {
            return gjs_value_to_basic_ghash_gi_argument(
                cx, value, key_type.tag(), value_type.tag(), transfer, arg,
                arg_name, arg_type, flags);
        }
        // else, fall through to generic marshaller
        ...
        if (element_type.is_basic()) {
            return gjs_value_to_basic_array_gi_argument(
                cx, value, element_type.tag(), type_info.array_type(), arg,
                arg_name, arg_type, flags);
        }
        // else, fall through to generic marshaller
    }
    ...
    case GI_TYPE_TAG_VOID:
        ...
        gjs_arg_unset(arg);
        return check_nullable_argument(cx, arg_name, arg_type, type_tag, flags,
                                       arg);
    case GI_TYPE_TAG_GLIST:
        return gjs_array_to_g_list(cx, value, type_info, transfer, arg_name,
                                   arg_type, &gjs_arg_member<GList*>(arg));
    case GI_TYPE_TAG_GSLIST:
        return gjs_array_to_g_list(cx, value, type_info, transfer, arg_name,
                                   arg_type, &gjs_arg_member<GSList*>(arg));
    ...
    case GI_TYPE_TAG_GHASH: {
        if (value.isNull()) {
            gjs_arg_set(arg, nullptr);
            if (!(flags & GjsArgumentFlags::MAY_BE_NULL)) {
                throw_invalid_argument(cx, value, type_info, arg_name,
                                       arg_type);
                return false;
            }
            return true;
    ...
    case GI_TYPE_TAG_ARRAY: {
        ...
        if (!gjs_array_to_explicit_array(cx, value, type_info, arg_name,
...
bool gjs_value_to_callback_out_arg(JSContext* cx, JS::HandleValue value,
    ...
    if (!arg) {
        gjs_throw(cx, "Return value {} is not optional but was passed NULL",
                  arg_info.name());
        return false;
    }
...
bool gjs_value_from_basic_gi_argument(JSContext* cx,
    ...
    gjs_debug_marshal(GJS_DEBUG_GFUNCTION,
                      "Converting GIArgument {} to JS::Value", type_tag);
    ...
    switch (type_tag) {
        ...
        case GI_TYPE_TAG_UNICHAR: {
            ...
            if (!g_unichar_validate(value)) {
                gjs_throw(cx, "Invalid unicode codepoint U+{:X}",
...
static bool gjs_array_from_basic_c_array_internal(
    ...
    switch (element_tag) {
        ...
        case GI_TYPE_TAG_FILENAME:
            ...
            break;
        case GI_TYPE_TAG_VOID:
            gjs_throw(cx, "Unknown Array element-type {}", element_tag);
            return false;
        default:
            g_assert_not_reached();
    }
...
GJS_JSAPI_RETURN_CONVENTION
static bool gjs_array_from_carray_internal(JSContext* cx,
    ...
    GITypeTag element_tag = element_type.tag();
    if (GI_TYPE_TAG_IS_BASIC(element_tag)) {
        return gjs_array_from_basic_c_array_internal(cx, value_p, element_tag,
                                                     length, array);
    }

    // a null array pointer takes precedence over whatever `length` says
    if (!array) {
        JSObject* jsarray = JS::NewArrayObject(cx, 0);
        if (!jsarray)
            return false;
    ...
    switch (element_tag) {
        ...
        case GI_TYPE_TAG_GHASH:
        case GI_TYPE_TAG_ERROR:
            if (!fill_vector_from_carray<void*>(cx, elems, element_type, &arg,
                                                array, length, transfer))
                return false;
            break;
        default:
            // Basic types handled above
            gjs_throw(cx, "Unknown Array element-type {}", element_type);
            return false;
    }
...
bool gjs_value_from_explicit_array(
    ...
    const GI::TypeInfo& type_info, GIArgument* arg, size_t length,
...
bool gjs_array_from_g_value_array(JSContext* cx, JS::MutableHandleValue value_p,
    ...
    if (g_type_is_a(value_gtype, G_TYPE_BYTE_ARRAY) ||
        ...
        g_assert_not_reached();
        gjs_throw(cx, "{} is not an array type", g_type_name(value_gtype));
        return false;
    }
...
static bool gjs_array_from_zero_terminated_c_array(
    ...
    if (element_type.is_basic()) {
        return gjs_array_from_basic_zero_terminated_array(cx, value_p,
                                                          element_tag, c_array);
    }
    ...
    switch (element_tag) {
        ...
        case GI_TYPE_TAG_ERROR:
            if (!fill_vector_from_zero_terminated_pointer_carray(
                    cx, elems, element_type, &arg, c_array, transfer))
                return false;
            break;
        default:
            // Handled in gjs_array_from_basic_zero_terminated_c_array()
            gjs_throw(cx, "Unknown element-type {}", element_type);
            return false;
    }
...
bool gjs_object_from_g_hash(JSContext* cx, JS::MutableHandleValue value_p,
    ...
    while (g_hash_table_iter_next(&iter, &key_pointer, &val_pointer)) {
        ...
        if (!gjs_value_from_gi_argument(cx, &keyjs, key_type,
                                        GJS_ARGUMENT_HASH_ELEMENT, transfer,
                                        &keyarg))
            return false;

        JS::RootedId key{cx};
        if (!JS_ValueToId(cx, keyjs, &key))
            return false;

        val_type.argument_from_hash_pointer(val_pointer, &valarg);
        if (!gjs_value_from_gi_argument(cx, &valjs, val_type,
                                        GJS_ARGUMENT_HASH_ELEMENT, transfer,
                                        &valarg) ||
            !JS_DefinePropertyById(cx, obj, key, valjs, JSPROP_ENUMERATE))
            return false;
    }
...
bool gjs_value_from_basic_fixed_size_array_gi_argument(
    ...
    gjs_debug_marshal(GJS_DEBUG_GFUNCTION,
                      "Converting GIArgument fixed array of {} to JS::Value",
                      element_tag);
...
bool gjs_value_from_basic_garray_gi_argument(JSContext* cx,
                                             ...
                                             GIArgument* arg) {
    gjs_debug_marshal(GJS_DEBUG_GFUNCTION,
                      "Converting GIArgument GArray of {} to JS::Value",
                      element_tag);
...
bool gjs_value_from_basic_gptrarray_gi_argument(
    ...
    GIArgument* arg) {
    gjs_debug_marshal(GJS_DEBUG_GFUNCTION,
                      "Converting GIArgument GPtrArray of {} to JS::Value",
                      element_tag);
...
bool gjs_value_from_gi_argument(JSContext* cx, JS::MutableHandleValue value_p,
                                const GI::TypeInfo& type_info,
                                GjsArgumentType argument_type,
                                GITransfer transfer, GIArgument* arg) {
    GITypeTag type_tag = type_info.tag();
    if (type_info.is_basic())
        return gjs_value_from_basic_gi_argument(cx, value_p, type_tag, arg);

    if (type_tag == GI_TYPE_TAG_GLIST) {
        GI::AutoTypeInfo element_type{type_info.element_type()};
        if (element_type.is_basic()) {
            return gjs_array_from_basic_glist_gi_argument(
                cx, value_p, element_type.tag(), arg);
        }
        // else fall through to generic marshaller
    }
    ...
    if (type_tag == GI_TYPE_TAG_GHASH) {
        ...
        GI::AutoTypeInfo value_type{type_info.value_type()};
        if (key_type.is_basic() && value_type.is_basic()) {
            return gjs_value_from_basic_ghash(cx, value_p, key_type.tag(),
                                              value_type.tag(),
    ...
    if (type_tag == GI_TYPE_TAG_ARRAY) {
        ...
        if (element_type.is_basic()) {
            switch (type_info.array_type()) {
                ...
                case GI_ARRAY_TYPE_BYTE_ARRAY:
                    return gjs_value_from_byte_array_gi_argument(cx, value_p,
                                                                 arg);
    ...
    switch (type_tag) {
        ...
        case GI_TYPE_TAG_INTERFACE: {
            ...
            if (interface_info.is_unresolved()) {
                gjs_throw(cx, "Unable to resolve arg type '{}'",
                          interface_info.name());
                return false;
            }
                ...
                if (interface_info.is_flags()) {
                    ...
                    if (gtype != G_TYPE_NONE) {
                        ...
                        if (!std::in_range<uint32_t>(value_int64)) {
                            gjs_throw(cx,
                                      "{:#x} is not a valid value for flags {}",
            ...
            if (auto struct_info = interface_info.as<GI::InfoTag::STRUCT>();
                ...
                if (g_type_is_a(gtype, G_TYPE_INTERFACE)) {
                    return gjs_lookup_interface_constructor(cx, gtype, value_p);
                }
                return gjs_lookup_object_constructor(cx, gtype, value_p);
            }
            ...
            if (auto struct_info = interface_info.as<GI::InfoTag::STRUCT>()) {
                if (GI::is_gdk_atom(*struct_info)) {
                    ...
                    if (result.isErr()) {
                        gjs_throw(cx, "Failed to call gdk_atom_name(): {}",
                                  result);
                        return false;
                    }
            ...
            if (auto union_info = interface_info.as<GI::InfoTag::UNION>()) {
                JSObject* obj = UnionInstance::new_for_c_union(
                    cx, union_info.value(), gjs_arg_get<void*>(arg));
                if (!obj)
                    return false;
            ...
            if (g_type_is_a(gtype, G_TYPE_OBJECT)) {
                ...
                return ObjectInstance::set_value_from_gobject(
                    cx, gjs_arg_get<GObject*>(arg), value_p);
            }
            ...
            if (g_type_is_a(gtype, G_TYPE_PARAM)) {
                JSObject* obj = gjs_param_from_g_param(
                    cx, G_PARAM_SPEC(gjs_arg_get<GParamSpec*>(arg)));
                if (!obj)
                    return false;
                value_p.setObject(*obj);
                return true;
            ...
            gjs_throw(cx,
                      "Unhandled GType {} packing GIArgument into JS::Value",
                      g_type_name(gtype));
            return false;
        }
    ...
    case GI_TYPE_TAG_ARRAY:
        ...
        if (type_info.array_type() == GI_ARRAY_TYPE_C) {
            ...
            return gjs_array_from_fixed_size_array(
                cx, value_p, type_info, transfer, gjs_arg_get<void*>(arg));
        }

        if (type_info.array_type() == GI_ARRAY_TYPE_BYTE_ARRAY) {
            auto* byte_array = gjs_arg_get<GByteArray*>(arg);
        ...
        return gjs_array_from_boxed_array(cx, value_p, type_info.array_type(),
                                          type_info.element_type(), transfer,
                                          arg);

    case GI_TYPE_TAG_GLIST:
        return gjs_array_from_g_list(cx, value_p, type_info, transfer,
                                     gjs_arg_get<GList*>(arg));
    case GI_TYPE_TAG_GSLIST:
        return gjs_array_from_g_list(cx, value_p, type_info, transfer,
                                     gjs_arg_get<GSList*>(arg));
    ...
    case GI_TYPE_TAG_GHASH:
        return gjs_object_from_g_hash(cx, value_p, type_info.key_type(),
...
static bool gjs_g_arg_release_internal(
        ...
        case GI_TYPE_TAG_INTERFACE: {
            ...
            if (auto struct_info = interface_info.as<GI::InfoTag::STRUCT>();
                ...
                return gjs_struct_foreign_release_gi_argument(
                    cx, transfer, struct_info.value(), arg);
            ...
            gjs_throw(cx, "Unhandled GType {} releasing GIArgument",
                      g_type_name(gtype));
            return false;
        }
        ...
        if (array_type == GI_ARRAY_TYPE_ARRAY) {
            ...
            switch (element_tag) {
                ...
                default:
                    // basic types handled above
                    gjs_throw(
                        cx, "Don't know how to release GArray element-type {}",
                        element_tag);
                    return false;
            }
        }
    ...
    case GI_TYPE_TAG_GLIST:
        return gjs_g_arg_release_g_list<GList>(cx, transfer, type_info, flags,
                                               arg);
...
bool gjs_gi_argument_release(JSContext* cx, GITransfer transfer,
    ...
    gjs_debug_marshal(GJS_DEBUG_GFUNCTION,
                      "Releasing GIArgument {} out param or return value",
                      type_info);
...
bool gjs_gi_argument_release_in_arg(JSContext* cx, GITransfer transfer,
    ...
    gjs_debug_marshal(GJS_DEBUG_GFUNCTION, "Releasing GIArgument {} in param",
                      type_info);
...
bool gjs_gi_argument_release_out_array(JSContext* cx, GITransfer transfer,
    ...
    return gjs_gi_argument_release_array_internal<
        ArrayReleaseType::ZERO_TERMINATED>(cx, element_transfer,
