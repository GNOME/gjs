...
import Gio, { IconNamespace } from 'gi://Gio';
import GLib from 'gi://GLib';
import GObject from 'gi://GObject';
import Gtk from 'gi://Gtk?version=3.0';

declare module 'gi://GLib' {
    export const MININT64_BIGINT: bigint;
    export const MINUINT64_BIGINT: bigint;
    export const MAXUINT64_BIGINT: bigint;
    export const MAXINT64_BIGINT: bigint;
}

declare module 'gi://GObject' {
    // export interface GType<T = unknown> { __type__(arg: never): T; name: string };
    export const TYPE_JSOBJECT: any;
    export class BindingGroup {
        constructor(params: any);
        bind(type: string, value: any, type2: string, flags: GObject.BindingFlags.DEFAULT | number);
        bind_full(type: string, value: any, type2: string, flags: GObject.BindingFlags.DEFAULT | number,
        cb: (bind, source) => [boolean, string],
        vals: null);
    }
    ...
    export module ParamSpec {
       ...
       export function jsobject(name: string, nick: string, desc: string, flags:  GObject.ParamFlags | number);
    }
}

// @ts-expect-error
const {gobject, property, signal} = GObject;
...
class MyObject extends GObject.Object {
    ...
    _readonly: any;
    _constructProp: any;
    _run_last_callback: any;
    empty_called: boolean;
    full_default_handler_called: boolean;

    @property.getter(GObject.ParamSpec.string('readwrite', 'ParamReadwrite',
    ...
    @property.getter(GObject.ParamSpec.string('readonly', 'ParamReadonly',
    ...
    @property.getter(GObject.ParamSpec.string('construct', 'ParamConstructOnly',
    ...
    @signal({name: 'minimal', param_types: [GObject.TYPE_INT, GObject.TYPE_INT]})
    emitMinimal(one, two) {
        this.emit('minimal', one, two);
    }

    @signal({name: 'full',flags: GObject.SignalFlags.RUN_LAST,
    ...
    @signal({name: 'detailed',     flags: GObject.SignalFlags.RUN_FIRST | GObject.SignalFlags.DETAILED,
...
describe('GObject class with decorators!', function () {
    let myInstance;
    beforeEach(function () {
    ...
    it('constructs with property values from Gtk.Builder', function () {
        ...
        let myInstance3 = builder.get_object<MyObject>('MyObject');
...
// @ts-expect-error
    ...
    it('are defined also for static virtual functions', function () {
        ...
        expect((CustomEmptyGIcon as unknown as Gio.IconNamespace).deserialize).toBe(Gio.Icon.deserialize);
        expect(Gio.Icon.new_for_string).toBeInstanceOf(Function);
        expect((CustomEmptyGIcon as unknown as Gio.IconNamespace).new_for_string).toBe(Gio.Icon.new_for_string);
    });
});
...
// @ts-expect-error
    ...
    it('use the file path and class name', function () {
        ...
        // @ts-expect-error
        expect(GTypeTestAutoName.$gtype.name).toEqual(
            'Gjs_esnext_testGObjectClass_GTypeTestAutoName');
    });
    ...
    it('use the file path and sanitized class name', function () {
        ...
        // @ts-expect-error
        expect(GTypeTestAutoName.$gtype.name).toEqual(
            'Gjs_esnext_testGObjectClass_GTypeTestAutoCla__Name');
    });
...
describe('Signal handler matching', function () {
    ...
    it('blocks a handler by callback', function () {// @ts-expect-error
...
describe('Property bindings', function () {
    ...
    it('can bind properties of the same type', function () {
        a.bind_property('string', b, 'string', GObject.BindingFlags.DEFAULT);
        a.string = 'foo';
    ...
    it('can use custom mappings to bind properties of different types', function () {
        a.bind_property_full('bool', b, 'string', GObject.BindingFlags.DEFAULT,
    ...
    it('can be set up as a group', function () {
        ...
        group.bind('string', b, 'string', GObject.BindingFlags.DEFAULT);
        a.string = 'foo';
        expect(a.string).toEqual('foo');
        expect(b.string).toEqual('foo');
    });
    ...
    it('can be set up as a group with custom mappings', function () {
        if (GObject.BindingGroup === undefined)
            pending('GLib version too old');
        const group = new GObject.BindingGroup({source: a});
        group.bind_full('bool', b, 'string', GObject.BindingFlags.DEFAULT,
            (bind, source) => [true, `${source}`],
            null);
        a.bool = true;
        expect(a.bool).toEqual(true);
...
describe('Auto accessor generation', function () {
    const AutoAccessors = GObject.registerClass({
        ...
        _constructOnlySetterCalled: number;
        _snakeNameGetterCalled: number;
        _snakeNameSetterCalled: number;
        _camelNameGetterCalled: number;
        _camelNameSetterCalled: number;
        _kebabNameGetterCalled: number;
        _kebabNameSetterCalled: number;
        _missingGetter: any;
        _constructOnlyValue: any;
        _init(props = {}) {
...
describe('GObject class with JSObject property', function () {
    ...
    it('property value survives when GObject wrapper is collected', function () {
        ...
        System.gc();
        // TODO: Add generics?
        expect((stream.get_converter() as typeof MyConverter['prototype']).testprop).toEqual([1, 2, 3]);
    });
});
...
describe('GObject class with int64 properties', function () {
    @gobject()
    class MyInt64Class extends GObject.Object {
        @property(GObject.ParamSpec.int64('int64', 'int64', 'int64',
