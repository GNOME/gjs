...
const handlers = {
     ...
     */
    launch(seq, args) {
        const filePath = buildFilename(args.cwd || '.', args.program);
    ...
    /**
     * @param {number} seq
     * @param {{ source: { path: string; name: string; }; breakpoints: {line: number}[]; sourceModified: boolean; }} args
     */
     ...
     *
     * @param {number} seq
     * @param {{granularity?: DAP.SteppingGranularity}} _args
     */
    next(seq, _args) {
        ...
        setUntilNextRequest(newestFrame, 'onPop', /** @this {Debugger.Frame} */ function () {
 ...
 */
function onStepped() {
    if (!isDebuggeeFrame(this))
        return;

    const location = getFrameLocation(this);

    if (isSameLocation(location, STATE.lastLocation))
        return;
    ...
    case 'object':
        if (variable === null) {
            ...
            if (variable.isPromise) {
                ...
                variableDescription = {
                    type: 'object',
                    value: variable.displayName ?? variable.name ?? '<unknown>',
                };
            }
            ...
            variableDescription = {type: 'object', value: '<unknown>'};
        }
        break;
    }
 ...
 * @param {number} _id
 */
function toDapScope(environment, _id = 0) {
    if (!environment)
        return null;
    ...
    if (kind)
        displayName = name ? `${kind}: ${name}` : kind;
...
/**
 * @param {Debugger.Frame | null} frame
 * @param {Debugger.Value} _value
 */
dbg.onExceptionUnwind = function (frame, _value) {
