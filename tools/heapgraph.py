    ...
    for target in args.targets:
        ...
        if addr_regex.match(target):
            if target in edges:
                ...
                targets += target_gobject(graph, target)
        else:
            # Fallback to looking for JavaScript objects by class name
            targets += target_type(graph, target)

    for target in args.edge_targets:
        targets += target_edge(graph, target)
    for target in args.func_targets:
        targets += target_func(graph, target)
    for target in args.string_targets:
        targets += target_string(graph, target)
